...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    // the old implementation of the melt fraction function
    // is commented out, because it is not used in the current implementation
    // it is kept here for reference, in case we want to use it again
    // template <int dim>
    // double
    // MeltThermodynamicEquilibrium<dim>::
          ...
          if (this->include_melt_transport())
            {
              ...
              if (include_melting_and_freezing && (in.requests_property(MaterialProperties::reaction_terms) ||
                                                   ...
                                                   in.requests_property(MaterialProperties::viscosity)))
                {
                  ...
                  // Calculate the equilibrium melt fraction.
                  // There is temporarily 2 compositions participating in the melting
                  // process: peridotite and carbonated peridotite.
    ...
    // the old implementation of the declare_parameters function
    // is commented out, because it is not used in the current implementation
    // now we need to declare some new parameters
    /*
    template <int dim>
    void
    MeltThermodynamicEquilibrium<dim>::declare_parameters (ParameterHandler &prm)
    ...
    */
    ...
    // the old implementation of the parse_parameters function
    // is commented out, because it is not used in the current implementation
    // now we need to parse some new parameters
    /*
    template <int dim>
    void
    MeltThermodynamicEquilibrium<dim>::parse_parameters (ParameterHandler &prm)
